...
def hard(players,way):
    import random
    # dhmiourgw mia lista pou anaparista ta stoixeia tou duskolou epipedou
    ...
    while w <= 52:
            ...
            while i <= players and w<=52:
                if f == True:
                    i = y # ksana paizei o idios paikths
                    f = False
                if f1 == True:
                    f1 = False
                    if i == players:
                ...
                if (card1[0] == 'Q' and card2[0] == 'K') or (card2[0] == 'Q' and card1[0] == 'K') and w<=52:
                    ...
                    for z in range(4):
                        print(str(z+1), '\t', x[z][0], '\t', x[z][1], '\t', x[z][2], '\t', x[z][3],'\t', x[z][4], '\t', x[z][5], '\t', x[z][6], '\t', x[z][7], '\t', x[z][8], '\t', x[z][9], '\t', x[z][10],'\t', x[z][11], '\t', x[z][12])
                    if card1[0] == card2[0] or card1[0] == card3[0] or card2[0] == card3[0]:
                        score_list[i-1] += score(card1,card2,way)
                        print('Επιτυχές ταίριασμα +', score(card1,card2,way), ' πόντοι! Παίκτη',i, 'έχεις συνολικά', score_list[i-1], ' πόντους.')
                    # stis parakatw periptswseis kleinw tis kartes pou den tairiazoun metaksu twn triwn  
                    if card1[0] == card2[0]:
